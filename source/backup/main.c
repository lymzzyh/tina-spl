...
//xxx #define SUNXI_UART0_BASE	0x01C28000
...
int sunxi_gpio_set_cfgpin(u32_t pin, u32_t val)
{
	u32_t cfg;
...
int sunxi_gpio_set_pull(u32_t pin, u32_t val)
{
	u32_t cfg;
	u32_t bank = GPIO_BANK(pin);
...
int sunxi_gpio_output(u32_t pin, u32_t val)
{
	u32_t dat;
	u32_t bank = GPIO_BANK(pin);
	u32_t num = GPIO_NUM(pin);
	struct sunxi_gpio *pio =
		&((struct sunxi_gpio_reg *)SUNXI_PIO_BASE)->gpio_bank[bank];
	dat = readl(&pio->dat);
	if(val)
...
int sunxi_gpio_input(u32_t pin)
{
	u32_t dat;
	u32_t bank = GPIO_BANK(pin);
	u32_t num = GPIO_NUM(pin);
...
u32_t sid_read_key(u32_t sid_base, u32_t offset)
{
	u32_t reg_val;
...
int soc_is_h2_plus(void)
{
	...
	u32_t sid0 = sid_read_key(SUN8I_SID_BASE, 0);
...
void gpio_init(void)
{
	if (soc_is_a10() || soc_is_a20() || soc_is_r40()) {
		...
		gpio_direction_input(SUNXI_GPE(0));
		gpio_direction_input(SUNXI_GPE(1));
		gpio_direction_input(SUNXI_GPC(3));
...
int get_boot_device(void)
{
	...
	u32_t boot_dev = spl_signature[9] & 0xFF; /* offset into SPL = 0x28 */
	if (boot_dev == 0)
